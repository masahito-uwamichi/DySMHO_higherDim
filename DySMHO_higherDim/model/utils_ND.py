...
def exp_using_variable(i):
    return lambda *args: exp(args[i])

def sin_using_variable(i):
    return lambda *args: sin(args[i])

def cos_using_variable(i):
    return lambda *args: cos(args[i])
...
def time_scale_conversion(t, horizon_length, optim_options, time_scale, y_measured): 
    ...
    N_dim = y_measured.shape[1]

    y_cols = []

    for k in range(N_dim):
        y_cols.append(data_interp(t_col,
                                  [i for i in time_scale if (i >= t) & (i <= t + horizon_length + 1e-2)],
                                  [y_measured[i, k] for i, j in enumerate(time_scale) if (j >= t) & (j <= t + horizon_length + 1e-2)]))
...
Formualates and solves NLP corresponding to one iteration of the MH discovery algorithm 
    Inputs: 
    - y_init: (vector Nx1) initial conditions for states at the beginning of the time horizon ###
    - t_span: (vecotr Nx1) time span for the optimization problem  ###
    ...
    - optim_options: options for discretization of continuous-time problem into discrete-time 
'''
def optim_solve(y_init, t_span, initial_theta, theta_bounds, y, basis, all_features_sym, iter_num, thresholded_indeces, optim_options, sign): ###
    ...
    def _diffeq1(m, i, t):
        ...
        for k in range(N_dim):
            for n in basis[k]['names']:
                if n[0:3] == 'exp':
                    basis[k]['functions'][basis[k]['names'].index(n)] = exp_using_variable(k)
                elif n[0:3] == 'sin':
                    basis[k]['functions'][basis[k]['names'].index(n)] = sin_using_variable(k)
                elif n[0:3] == 'cos':
    ...
    m.theta_sum_diff_sq_c = Constraint(expr=sum((m.theta[i] - m.theta_init[i]) ** 2 for i in m.params) <= m.theta_sum_diff_sq)

...
# Defining dynamcis
def dy_dt(y, t):
    dy_dt = np.array([10*(y[1]-y[0]), y[0]*(28-y[2])-y[1], y[0]*y[1]-(8/3)*y[2], -y[3]**3+2*y[3]**2-y[3]+0.5+np.sin(y[3])])
...
def dyn_sim(theta, xs, initial_conditions, y, plot):
    ...
    def dy_dt_sim(y, t):
        ...
        for i in range(0, len(y)):
            ...
            if i == 3:
                dy_dt_sim[3] = theta[33] * y[3]**3 + theta[34] * y[3]**2 + theta[35] * y[3] + theta[36] + theta[37] * np.sin(y[3])
    ...
    if plot == True:
        ...
        plt.plot(xs, y[:, 3], 'o', color='#4575b4')
        plt.plot(xs_int, ys[:, 3], color='black')
        plt.ylim((-30, 50))
        plt.pause(0.01)
...
def data_gen(xs, initial_conditions, noise, plot):
    ...
    ys[1:, 2] = ys[1:, 2] + np.random.normal(noise[4], noise[5], len(ys[1:, 2]))
    ys[1:, 3] = ys[1:, 3] + np.random.normal(noise[6], noise[7], len(ys[1:, 3]))

    if plot == True:
        plt.plot(xs, ys[:, 0], color='#d73027')
        plt.plot(xs, ys[:, 1], color='#fc8d59')
        plt.plot(xs, ys[:, 2], color='#fee090')
        plt.plot(xs, ys[:, 3], color='#4575b4')
        plt.draw()
...
def derivative_eval(y_init, t_span, n, y, nfe, ncp):
    ...
    dx4dt = np.array([value(m.dxdt[4, t]) for t in m.t])
    dxdt = np.column_stack((dx1dt, dx2dt, dx3dt, dx4dt))
